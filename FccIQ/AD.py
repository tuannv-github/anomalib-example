...
class AE(nn.Module):
    def __init__(self, input_channels=2, latent_dim=2**12, img_height=300, img_width=14):
        ...
        self.fc_offset = nn.Linear(self.feature_dim, 128)

        # Decoder
        self.decoder_input = nn.Linear(latent_dim + 128, self.feature_dim)
        ...
        recon, _ = self.forward(test_input)
    ...
    def encode(self, x):
        ...
        offset = self.fc_offset(h)
        return mu, logvar, offset
    ...
    def decode(self, z, offset):
        h = self.decoder_input(torch.cat([z, offset], dim=1))
        return self.decoder(h)

    def forward(self, x):
        mu, logvar, offset = self.encode(x)
        z = self.reparameterize(mu, logvar)
        recon_x = self.decode(z, offset)
        return recon_x, (mu, logvar, offset)

if __name__ == "__main__":
    ae = AE(input_channels=2, latent_dim=1024, img_height=300, img_width=14)
    ae.eval()
    with torch.no_grad():
        recon, _ = ae(torch.randn(1, 2, 300, 14))
